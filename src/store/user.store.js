...
export const userStore = {
  ...
  mutations: {
    ...
    setUser(state, { user, optimistic = false }) {
      state.optimisticUser = optimistic ? state.loggedinUser : null
  ...
  actions: {
    ...
    async updateUser({ commit, state }, { user }) {
      try {
        commit({ type: 'setUser', user, optimistic: true })
        user = await userService.update(user)
        commit({ type: 'setUser', user })
      } catch (err) {
        console.log('userStore: Error in updateUser', err)
        commit({ type: 'setUser', user: state.optimisticUser })
        throw err
      }
    },
